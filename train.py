...
def draw():
    ...
    # draw the action button (grayed out when train is in motion)
    if state == State.STOPPED:
        ...
        screen.draw.filled_rect(button, (160, 160, 160))
        screen.draw.rect(button, (100, 100, 100))
        screen.draw.text("DEPART", center=button.center, fontsize=32,
                         color=(210, 210, 210))

    # nyc station "left"
    screen.draw.filled_rect(Rect(STATION_NYC_X - 60,
                                 TRACK_Y - 55, 120, 55), (160, 160, 160))
    screen.draw.text("New York City", center=(STATION_NYC_X, TRACK_Y - 70),
                     fontsize=28, color="Dark Blue")
    ...
    screen.draw.text(f"Train to {destination}", topleft=(20, 20),
                     fontsize=24, color="Black")
    screen.draw.text(f"Distance from New York City {dist_from_nyc:.0f}",
                     topleft=(20, 50), fontsize=24, color="Black")
    screen.draw.text(f"Distance from Miami {dist_from_miami:.0f}",
                     topleft=(20, 80), fontsize=24, color="Black")
    screen.draw.text(f"Speed: {train_speed * MPH_SCALE:.0f} MPH",
                     topleft=(20, 110), fontsize=24, color="Black")
...
def update():
    ...
    # Nothing to update if the train is stopped
    if state == State.STOPPED:
        return
    ...
    if target_station == Destination.MIAMI:
        # calculate the new train position and
